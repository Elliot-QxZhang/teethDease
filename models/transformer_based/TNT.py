...
def create_tnt(image_size=224, patch_dim=512, pixel_dim=24, patch_size=16, pixel_size=4, depth=6, num_classes=2, attn_dropout=0.1, ff_dropout=0.1):
    return TNT(
        image_size=image_size,  # size of image
        patch_dim=patch_dim,  # dimension of patch token
        pixel_dim=pixel_dim,  # dimension of pixel token
        patch_size=patch_size,  # patch size
        pixel_size=pixel_size,  # pixel size
        depth=depth,  # depth
